...
const AdminResponses = () => {
    ...
    const handleResponseSubmit = async (id, response, index) => {
        if (!response || !response.trim()) {
            const updatedReviews = [...reviews];
            updatedReviews[index].errorMessage = 'Please add a response before submitting.';
            setReviews(updatedReviews);
            return;
        }
        
        try {
            await axios.put(`/api/review/${id}`, { response });
            setShowPopup(true); // Show pop-up message
            const updatedReviews = [...reviews];
            updatedReviews[index].errorMessage = ''; // Clear error message
            setReviews(updatedReviews);
        } catch (error) {
